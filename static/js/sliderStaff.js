...
const firstCardStaffWidth = carouselStaff.querySelector('.cardStaff').offsetWidth
...
let cardStaffPerView = Math.round(carouselStaff.offsetWidth / firstCardStaffWidth)
...
carouselStaffChildrens
    ...
    .forEach((cardStaff, index) => {
        ...
        clonedCardStaff.classList.add('infinite-cardStaff', `infinite-cardStaff-${index}`)
        carouselStaff.insertAdjacentElement('afterbegin', clonedCardStaff)
    })

// Insert copies of the first few cardStaffs to the end of the carouselStaff for infinite scrolling
carouselStaffChildrens.slice(0, cardStaffPerView).forEach((cardStaff, index) => {
    const clonedCardStaff = cardStaff.cloneNode(true)
    clonedCardStaff.classList.add('infinite-cardStaff', `infinite-cardStaff-${index}`)
    carouselStaff.insertAdjacentElement('beforeend', clonedCardStaff)
})
...
arrowBtns.forEach((btn) => {
    btn.addEventListener('click', () => {
        carouselStaff.scrollLeft +=
            btn.id == 'left' ? -firstCardStaffWidth : firstCardStaffWidth
    })
})
...
const infiniteScroll = () => {
    ...
    if (carouselStaff.scrollLeft === 0) {
        carouselStaff.classList.add('no-transition')
        carouselStaff.scrollLeft = carouselStaff.scrollWidth - 2 * carouselStaff.offsetWidth
        carouselStaff.classList.remove('no-transition')
    }
...
const autoPlay = () => {
    ...
    timeoutId = setTimeout(() => (carouselStaff.scrollLeft += firstCardStaffWidth), 2500)
}
autoPlay()
...
wrapperStaff.addEventListener('mouseenter', () => clearTimeout(timeoutId))
